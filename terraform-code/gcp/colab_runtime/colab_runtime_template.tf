resource "google_colab_runtime_template" "runtime-template" {
  ...
  dynamic "encryption_spec" {
    for_each = local.colab_runtime_template_kms_key_names[each.key] != null ? [local.colab_runtime_template_kms_key_names[each.key]] : []
    content {
      kms_key_name = encryption_spec.value
    }
  }

  depends_on = [google_kms_crypto_key_iam_member.colab_runtime_template]
}