...
- `data.google_storage_project_service_account.cloudbuild_bucket` — *(só com CMEK)* resolve a service account do GCS do projeto.
- `google_kms_crypto_key_iam_member.cloudbuild_bucket` — *(só com CMEK)* concede `roles/cloudkms.cryptoKeyEncrypterDecrypter` na chave à service account do GCS, um binding por projeto.
...
- **CMEK do bucket padrão do Cloud Build**: `kms_project_id`, `kms_key_ring` e `kms_crypto_key` são opcionais, mas precisam vir juntos (validado em `variables.tf`) — quando informados, o módulo monta `kms_key_name = "projects/<kms_project_id>/locations/<location>/keyRings/<kms_key_ring>/cryptoKeys/<kms_crypto_key>"` e aplica no bloco `encryption` do bucket (a location da chave sempre reaproveita a `location` do pool, não é um campo separado). Sem CMEK informado, nenhum recurso de IAM é criado e o bucket não muda.
- **Permissão na chave do bucket (criada pelo módulo quando há CMEK)**: o módulo resolve a service account do GCS de cada projeto com CMEK (`data.google_storage_project_service_account`, em `data.tf`, `service-<PROJECT_NUMBER>@gs-project-accounts.iam.gserviceaccount.com`) e concede a ela `roles/cloudkms.cryptoKeyEncrypterDecrypter` na chave (`google_kms_crypto_key_iam_member.cloudbuild_bucket`, em `iam.tf`); o bucket tem `depends_on` nesse binding. Quem roda o `apply` precisa poder conceder IAM na chave KMS; se o primeiro `apply` falhar por acesso à chave logo após criar o binding, aguarde a propagação e rode de novo.
- **Por que o IAM do worker pool/SA é aditivo e por projeto**: os `google_project_iam_member` de `iam.tf` (`worker_pool_user` e `cloudbuild_sa_roles`) são aditivos porque o consumo do pool é feito por identidades externas ao módulo e as roles da SA vivem em recursos (BigQuery, GCS, Artifact Registry etc.) que este módulo não possui.
