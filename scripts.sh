...
#
# SEM --partition-key aqui de propósito: confirmado via debug (--log-http)
# que era essa flag que causava o INVALID_ARGUMENT quando rodando pelo
# Cloud Run (mesmo comando funcionava normalmente na VM). As tabelas já
# foram criadas particionadas anteriormente via VM, então o export sem essa
# flag deve continuar escrevendo nelas normalmente.
EXPORT_CMD="gcloud asset export --organization=${ORG_ID} --content-type=resource --bigquery-table=projects/${PROJECT_ID}/datasets/${DATASET}/tables/${TABLE_RESOURCES} --output-bigquery-force && gcloud asset export --organization=${ORG_ID} --content-type=iam-policy --bigquery-table=projects/${PROJECT_ID}/datasets/${DATASET}/tables/${TABLE_IAM} --output-bigquery-force"
