...
# Google APIs Service Agent / Cloud Services (<num>@cloudservices..., SEM o prefixo
# "service-", diferente dos demais service agents por API) precisam da role
# no projeto host da VPC.
locals {
  shared_vpc_bindings = merge([
    for k, v in var.dataproc_cluster_settings : v.shared_vpc_host_project == null ? {} : {
      for suffix, sa_email in {
        ...
        cloudservices      = "${data.google_project.shared_vpc_service_project[k].number}@cloudservices.gserviceaccount.com"
      } : "${k}-${suffix}" => {
  ...
  additional_bindings = merge([
    for k, v in var.dataproc_cluster_settings : {
