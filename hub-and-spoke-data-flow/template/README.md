...
- **`PAUSE_SCHEDULE`** / **`RESUME_SCHEDULE`** — suspende/retoma o agendamento sem apagar os recursos: grava `disabled: true`/`false` em `releaseConfigs/release-diaria` **e** `workflowConfigs/agendamento-diario` (campo dedicado da API do Dataform pra isso — o `cronSchedule` continua intacto, `disabled` é quem efetivamente liga/desliga a criação automática de compilações/execuções).
