...
PROJECT_ID = env_config.get('project_id', "prj-subscriber-poc")
DATASET_ID = env_config.get('dataset_id', "served")
LOCATION = "southamerica-east1"
PIPELINE_ROOT = env_config.get('pipeline_root', "gs://bucket-data-subs-des/pipeline_root")
SERVICE_ACCOUNT = env_config.get('service_account', "sa-vertex-ai-pipeline@prj-subscriber-poc.iam.gserviceaccount.com")

PIPELINE_NAME = model_training_config.get('pipeline_name', "credpj-risk-xgb-pipeline")
SOURCE_DIR = model_training_config.get('source_dir', "src")

# Parâmetros dinâmicos de Serving para o compilador
ENDPOINT_NAME = serving_config.get('endpoint_name', 'endpoint-credpj-risk')
...
def deploy_models_to_endpoint_step(
    ...
    # Determina a sigla do ambiente (prd ou des) compativel com a Landing Zone
...
@dsl.pipeline(
    name=PIPELINE_NAME,
    description='Pipeline de MLOps completo (Treinamento BQML XGBoost + Serving via SDK) para risco de credito PJ do Coffer Bank orquestrado via Vertex AI Pipelines.'
)
def mlops_pipeline(
    project_id: str,
    dataset_id: str,
...
if __name__ == '__main__':
    ...
    if args.submit:
        ...
        job = aiplatform.PipelineJob(
            display_name=f"credpj-risk-mlops-run-{args.environment}",
