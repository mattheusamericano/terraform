...
#
module "cloud_run" {
    source = "../../tf-modules-for-gcp/cloud_run"

    cloud_run_settings = var.enabled_cloud_run == true ? {for k, v in var.cloud_run_settings : k => merge(v, { labels = local.common_labels }) } : {}
...
module "dataproc_cluster" {
    source      = "../../tf-modules-for-gcp/dataproc_cluster"
    dataproc_cluster_settings = var.enabled_dataproc_cluster == true ? {for k, v in var.dataproc_cluster_settings : k => merge(v, { labels = local.common_labels }) } : {}    
...
#
module "dataproc_session_template" {
    source      = "../../tf-modules-for-gcp/dataproc_session_template"
    dataproc_session_template_settings = var.enabled_dataproc_session_template == true ? {for k, v in var.dataproc_session_template_settings : k => merge(v, { labels = local.common_labels }) } : {}    
...
module "cloudbuild_worker_pool" {
    source      = "../../tf-modules-for-gcp/cloudbuild_worker_pool"
    worker_pool_settings = var.enabled_cloudbuild_worker_pool == true ? {for k, v in var.worker_pool_settings : k => merge(v, { labels = local.common_labels }) } : {}    
