...
- **`main.tf`** — `module "iam"` com `for_each = var.ml_ops_settings` (uma instância do módulo `iam_new` por entrada do mapa — normalmente uma só, um projeto por ambiente, mas o mapa permite mais de um projeto no mesmo stack se algum ambiente precisar), passando `ml_ops_profiles` (repassa `environment_type` e os 3 grupos de `each.value`) e `iam_bindings = var.extra_group_role_bindings`.
- **`tfvars/terraform.tfvars`** — `ml_ops_settings` com uma entrada (chave = sigla do projeto), `environment_type` como `"nprod"` ou `"prod"`. Não inclui `extra_group_role_bindings` (default `{}`) — ver "Reativando o grant do RISCFAB" abaixo se este ambiente precisar dele.
...
## Reativando o grant do RISCFAB

O módulo `iam` antigo tinha um grupo fixo (`G_GCP_RISCFAB_DTSC@corp.caixa.gov.br`) hardcoded no código, recebendo `roles/iam.dataScientist` e `roles/aiplatform.admin` em todo ambiente que usasse o módulo. Em `iam_new`/`arch-iam_new` isso não existe mais — é opt-in e explícito por ambiente, via `extra_group_role_bindings`. Para reproduzir o acesso, adicione em `tfvars/terraform.tfvars`:
...
- Decidir, por ambiente, se o grant do grupo RISCFAB precisa ser reativado (ver seção acima).
